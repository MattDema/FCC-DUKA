#!/bin/bash
# =============================================================================
#  DUKA CLUSTER AUTO-HEAL SCRIPT — v2.0 (Calico Edition)
#  Run from: OpenNebula host (as oneadmin or with sudo)
#
#  Fixes applied on every boot:
#    - VM resume + bridge recreation
#    - Worker DNS (172.16.100.1 as resolver)
#    - Calico /26 pod CIDR routes on OpenNebula host
#    - BGP iptables exemptions (port 179)
#    - Flannel ghost rule cleanup on all nodes
#    - Calico internet masquerade (natOutgoing)
#    - Control plane routes to worker pod CIDRs
# =============================================================================
...
ssh_node()  { ssh $SSH_OPTS ubuntu@"$1" "${@:2}"; }
...
# =============================================================================
section "PHASE 5 — BGP NAT Exemptions (iptables port 179)"
# =============================================================================

# Ensure BGP traffic is never masqueraded on the OpenNebula host
for dir in dport sport; do
    sudo iptables -t nat -D POSTROUTING -p tcp --"$dir" 179 -j RETURN 2>/dev/null || true
done
sudo iptables -t nat -I POSTROUTING 1 -p tcp --dport 179 -j RETURN
sudo iptables -t nat -I POSTROUTING 1 -p tcp --sport 179 -j RETURN
ok "BGP exemptions set."

# Internet masquerade — allows pods to reach the internet (needed for Falco
# rule downloads, image pulls via kubelet, CoreDNS upstream resolution, etc.)
#
# Rule A: control plane network (172.16.100.0/24) → internet
#   Covers CoreDNS pods masqueraded to 172.16.100.2 going out to the internet
# Rule B: worker node network (10.1.0.0/16) → internet
#   Covers worker pods masqueraded to 10.1.x.100 going out to the internet
if ! sudo iptables -t nat -C POSTROUTING \
    -s 172.16.100.0/24 ! -d 172.16.100.0/24 -j MASQUERADE 2>/dev/null; then
    sudo iptables -t nat -A POSTROUTING \
        -s 172.16.100.0/24 ! -d 172.16.100.0/24 -j MASQUERADE
fi
if ! sudo iptables -t nat -C POSTROUTING \
    -s 10.1.0.0/16 ! -d 172.16.100.0/24 -j MASQUERADE 2>/dev/null; then
    sudo iptables -t nat -A POSTROUTING \
        -s 10.1.0.0/16 ! -d 172.16.100.0/24 -j MASQUERADE
fi
ok "Internet masquerade rules set."
...
for node_ip in "${ALL_NODE_IPS[@]}"; do
    for attempt in $(seq 1 30); do
        if ssh $SSH_OPTS ubuntu@"$node_ip" "true" 2>/dev/null; then
...
for node_ip in "${ALL_NODE_IPS[@]}"; do
    ...
    ssh_node "$node_ip" "
        ...
        # Add Calico internet masquerade (natOutgoing=true)
        # Only add if not already present
        if ! sudo iptables -t nat -C POSTROUTING \
            -s 10.244.0.0/16 ! -d 10.244.0.0/16 \
            -m comment --comment 'calico nat outgoing' -j MASQUERADE 2>/dev/null; then
            sudo iptables -t nat -A POSTROUTING \
                -s 10.244.0.0/16 ! -d 10.244.0.0/16 ! -o tunl0 \
                -m comment --comment 'calico nat outgoing' -j MASQUERADE
        fi
